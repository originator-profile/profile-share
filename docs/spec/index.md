...
この文書は Originator Profile 仕様の草案です。
関係者・エンジニア向けに検討段階・実証実験用プロトタイプの仕様を記載するものであり、例えば次のような点において最終的な仕様とは異なる可能性があります。
...
Originator Profile とは、Web コンテンツの出所と資格情報を検証可能とすることで、デジタル空間の信頼性向上を目的として仕様策定とプロトタイプ実装が進められている技術です。

Originator Profile では、Web コンテンツの「1. 対象範囲を特定」し、その作成/管理者の「2. 組織の基本情報」と「3. 組織の資格情報」を検証可能な形で付与することを通じて、Web の後方互換性を維持しつつ信頼性と透明性を高めます。

1. 対象範囲を特定
   - Web サイト、ページ、ページの一部などに、署名やドメイン認証によって検証可能(詐称不可能)な形で紐付ける
2. 組織の基本情報
   - サイト運営者や記事作成者などの組織の基本情報をレジストリに登録し、第三者確認済みの検証可能な情報として記載
   - レジストリ登録時に組織実在性など基本的な確認を受けるため OP ID 保有組織を信頼性のベースラインとして使える
3. 組織の資格情報
   - 第三者認証や所属組織など任意の「資格情報」を含め、適正コンテンツの提供元であるか、広告取引の不正対策をしているかなど応用分野や業界あるいは個別の取引毎に求める要件を満たす組織の判別を可能とする
   - ブラウザでの利用以外に広告取引システムや検索エンジンによる利用など含め OP 利用領域を広げる
...
Originator Profile では主に次のような検証可能なデータを作成・利用します。

- OP ID
  - 組織の主ドメインを識別子として OP レジストリに登録する ID
  - 組織の情報や検証鍵は OP レジストリに登録し誰でも参照可能とする
- OP (Originator Profile)
  - 組織の基本情報と資格情報を記載したプロファイル
  - 組織情報の記載内容を確認の上で OP レジストリ運営者が署名する
- DP (Document Profile)
  - サイトやコンテンツの基本情報を記載したプロファイル
  - サイト運営者やコンテンツ作者がその情報を記載し自己署名する

DP ではサイトやコンテンツの情報を記載しますが、その運営者や作成者は OP ID として記載し、必ず対応する OP を参照することで「Web コンテンツの出所と資格情報を検証可能」となります。

DP の対象となるコンテンツに対する直接的な資格情報を検証可能とするのではなく、コンテンツの出所を明らかとした上でその出所組織に対する資格情報を検証可能としています。信頼性確認としては間接的なモデルであることには注意が必要です。

:::note

DP として扱う対象としてはサイト、ページ、記事やメディアファイル、広告などがあり、その対象に応じてスキーマや取り扱いは異なります。

DP を第三者署名とすることや、自己署名情報に加えて第三者による評価・検証情報などを紐付けることで個別 DP に対してより直接的な信頼性確認を可能とする事は将来検討として、現仕様には含めていません。
...
データモデルの詳細な構造は [Profile Model](./model/) を参照してください。
...
## Document Profile データモデル {#document-profile-data-model}
...
必ず UUID 文字列表現 (RFC 9562) でなければなりません (MUST)。
...
### `dp` (Document Profile) クレーム {#dp-claim}
...
- `proof` プロパティは、対象のテキストへの署名を表すオブジェクトです。そのオブジェクトには必ず `jws` プロパティを持たなければなりません。その `jws` プロパティには [Compact Serialization](https://www.rfc-editor.org/rfc/rfc7515#section-7.1) でシリアライズした JWS を入れなければなりません。JWS には必ず [RFC 7797](https://www.rfc-editor.org/rfc/rfc7797) の Unencoded Payload Option を付与し、同 RFC の[Section 5.1](https://www.rfc-editor.org/rfc/rfc7797#section-5.1) に則りペイロード部は JWS から省略しなければなりません。
...
`visibleText` 型を利用する場合、署名対象のコンテンツが**ブラウザーによって変わらない**ように注意してください。署名検証時に、`innerText` 属性を利用して[レンダリングされた](https://html.spec.whatwg.org/multipage/rendering.html#being-rendered)テキストを取得するため、ブラウザ間のレンダリングの違いによって取得するコンテンツに違いが発生する場合があります。

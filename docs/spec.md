...
メディア・広告などに関わる組織の身元を表します。

### `credential` 型

認証機構によって報告された資格情報を表します。
...
対象の要素のその子孫のレンダリングされたテキストの内容への署名を表します。
...
対象の要素の子孫のテキストへの署名を表します。`type` プロパティとシリアライズの手続きが異なること以外 `visibleText` 型と同じ構造です。

### `html` 型

対象の要素とその子孫を含む部分を HTML としてシリアライズしたものへの署名を表します。`type` プロパティとシリアライズの手続きが異なること以外 `visibleText` 型と同じ構造です。

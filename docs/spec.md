...
- `location` プロパティは、対象の要素の場所を特定する CSS セレクターです。対象の要素は、そのページの `document` のルート要素 (例えば、 HTML 文書の場合は `<html>` 要素) から、`querySelectorAll()` メソッドを使用して検索します。
...
1. `url` プロパティの URL と `location` プロパティの CSS セレクターで指定した要素を検索します。もし仮に `location` プロパティが存在しない場合は、そのページの `document` のルート要素 (例えば、HTML 文書の場合は `<html>` 要素) を検索します。

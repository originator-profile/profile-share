...
DP の登録に成功すると、登録した DP を記事から参照することができます。これは、後述するもう一方のエンドポイントの URL を記事ページの HTML 内に `<link>` 要素で指定することで実現します（後述）。
...
| パラメータ名 | 型                |                                                                                                                    | 説明 |
| ------------ | ----------------- | ------------------------------------------------------------------------------------------------------------------ | ---- |
| id           | 文字列            | 記事の ID （DP の`main`プロパティ）を与えてください。必ず UUID 文字列表現 (RFC 4122) でなければなりません (MUST)。 |
...
間違ったリクエスト: `https://oprdev.originator-profile.org/website/profiles?url=https%3A%2F%2Fexample.com%2F`

逆に、登録時に URL `https://example.com/` を与えた場合は、 `https://oprdev.originator-profile.org/website/profiles?url=https%3A%2F%2Fexample.com%2F` が、正しいリクエストになります。

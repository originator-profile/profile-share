...
CMS 上での記事の追加・更新と連動して SDP を発行するシステムを開発する場合、このようなシステム構成で DP Store とやりとりするシステムを開発することを想定しています。

![CMS OP 連携のシステム構成図](./assets/cms.jpeg)

このようなシステム構成の場合、次のシーケンス図の流れでユーザー、OPシステム、ブラウザ、CMS が連動することでしょう。
...
実装においては OP CIP が開発したサービス・ツールを利用することで実装コストを下げられる可能性があります。

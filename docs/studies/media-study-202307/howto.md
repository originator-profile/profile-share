...
`image` プロパティを指定することで画像を拡張機能に表示することができます。詳細は [画像の表示](image.md) を参照してください。
...
API の詳細については、[CIP 提供 DP レジストリについて](#dp-registry-api) も参照ください。
...
{
  "input": {
    "//": "記事の ID を渡してください。必ず UUID 文字列表現 (RFC 9562) でなければなりません",
    "id": "403cc6d4-53d6-4286-9f42-930e0bf7bd3f",

    "//": "記事の URL です。絶対 URL でなければなりません。",
...
表示のされ方については[画像の表示](image.md)を参照してください。
...
[レジストリの管理者を作成](/registry/integration/document-profile-registry-creation.md#admin-create)した際の認証情報を、WordPress 管理者画面 > Settings > Profile > [認証情報] に入力します。
...
##### 署名をする記事コンテンツを選択する {#select-visibletext}
...
- [visibleText 型](/spec/index.md#visibletext-type)
- [text 型](/spec/index.md#text-type)
- [html 型](/spec/index.md#html-type)
...
登録する SDP と ID の等しい SDP がすでに DP レジストリに存在する場合には、既存の SDP を上書き更新します。そのため、 SDP 修正時もこのエンドポイントを利用して SDP の更新ができます。詳細は[記事を更新した場合](./sdp-case-study.md#on-update)を参照してください。
...
## CIP 提供 DP レジストリについて {#dp-registry-api}
...
### `/admin/publisher/{アカウントID}/dp/` エンドポイント詳細 {#sdp-register-endpoint}

このエンドポイントについては[API ドキュメント](pathname:///api/#operation/registerSignedDocumentProfile)も参照してください。
...
失敗レスポンス（`jwt` パラメータが [dp クレーム](/spec/index.md#dp-claim) を含んでいなかった場合）:
...
失敗レスポンス（`dp` クレームの [item プロパティ](/spec/index.md#dp-item) の中にコンテンツへの署名を含む要素がなかった場合）:
...
### `/website/profiles` エンドポイント詳細 {#website-profiles-endpoint}

このエンドポイントについては[API ドキュメント](pathname:///api/#operation/website.getProfileSet)も参照してください。
...
| パラメータ名 | 型                | 説明                                                                                      |
| ------------ | ----------------- | ----------------------------------------------------------------------------------------- |
| id           | 文字列            | 記事の ID を与えてください。必ず UUID 文字列表現 (RFC 9562) でなければなりません (MUST)。 |
...
上のエラーメッセージの場合、具体的には `jwt` の中に [dp クレーム](/spec#dp-claim) が含まれていないためエラーになっています。

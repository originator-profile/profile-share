...
- web \*\*\*
  - DOM の文脈におけるコンテンツやウェブで配信されるコンテンツを取り扱うので web というのは違和感はない
- manifest
  - C2PA manifest とかも利用する場合には名前の競合が発生する懸念
  - [Web Application Manifest](https://www.w3.org/TR/appmanifest/) もある
- content binding
  - key や sub を content に紐付けるという意味で類推
- assertion
  - W3C spec との被りはなさそう https://www.w3.org/TR/?filter-tr-name=assert
- statement
  - デジタルアイデンティティ関連の他の仕様など踏まえて類推
- signed statement
  - 旧称 signed document profile を踏まえ、またデジタルアイデンティティ関連の他の仕様など踏まえて類推
- annotation
  - [Web Annotation](https://w3c.github.io/web-annotation/) との混同は懸念
  - コンテンツの範囲を意味するという文脈からすると抽象的すぎる懸念

:::

## 概要

本文書では、JSON Web Token (JWT) を使用して、Web コンテンツの範囲を表明する方法について説明します。

JWT には、発行者によって署名された Web コンテンツの範囲に関する情報が含まれます。
...
```json
{
  "$schema": "https://example.org/web-assertion",
  "iss": "dns:example.com",
...
```json
{
  "$schema": "https://example.org/wa-advertisement",
  "iss": "dns:example.com",
  "sub": "78550fa7-f846-4e0f-ad5c-8d34461cb95b",
  "iat": 1680274800,
  "exp": 1743433200,
...
この WA は https://media.example.com 配下で公開されている Web Content に紐づいています。

```json
{
  "$schema": "https://example.org/wa-website",
  "iss": "dns:example.com",
  "sub": "78550fa7-f846-4e0f-ad5c-8d34461cb95b",
  "iat": 1680274800,
  "exp": 1743433200,

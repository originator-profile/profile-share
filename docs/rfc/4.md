...
REQUIRED. SD-JWT VC のタイプの識別子です。 [SD-JWT VC Retrieving Type Metadata](https://www.ietf.org/archive/id/draft-ietf-oauth-sd-jwt-vc-04.html#name-retrieving-type-metadata) に従い、`https://<authority>/.well-known/vct/<type>` への GET リクエストで Type Metadata を取得できなければなりません (MUST)。検証者はクレームの型を指定する JSON Schema や、選択的開示可能かどうか、各クレームの情報の正しさが誰によって確認されたかを知ることができます。

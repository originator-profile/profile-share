...
[Profile Set 取得エンドポイント](./howto.md#websiteprofiles-エンドポイント) は、クエリパラメータ `url` にマッチする SDP を Profile Set にして返します。 DP ID が異なる SDP が複数あった場合は、全てが Profile Set に含まれます。

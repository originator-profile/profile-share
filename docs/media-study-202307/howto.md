...
[記事を更新した場合](sdp-case-study.md#記事を更新した場合)を参照してください。
...
登録する SDP と ID の等しい SDP がすでに DP レジストリに存在する場合には、既存の SDP を上書き更新します。そのため、 SDP 修正時もこのエンドポイントを利用して SDP の更新ができます。詳細は[記事を更新した場合](./sdp-case-study.md#記事を更新した場合)を参照してください。
...
失敗レスポンス（`jwt` パラメータが [dp クレーム](/spec.md#dp-document-profile-クレーム) を含んでいなかった場合）:
...
失敗レスポンス（`dp` クレームの [item プロパティ](/spec.md#dp-クレームの-item-プロパティ) の中にコンテンツへの署名を含む要素がなかった場合）:

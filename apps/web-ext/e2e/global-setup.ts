...
// 現在のブラウザ言語設定を取得
function getCurrentLanguage(browser: "chromium" | "chrome"): string | undefined {
  const domain = browser === "chromium" ? "org.chromium.Chromium" : "com.google.Chrome";
  try {
    return execSync(
      `defaults read ${domain} AppleLanguages 2>/dev/null`,
      { encoding: "utf-8" }
    ).trim();
  } catch {
    return undefined;
...
// バックアップファイルを保存
function saveBackup(browser: "chromium" | "chrome", currentLanguages: string | undefined): void {
...
// ブラウザの言語設定をセットアップ
function setupBrowserLanguage(browser: "chromium" | "chrome", targetLocale: string): void {
  const browserName = browser === "chromium" ? "Chromium" : "Chrome";

  const currentLanguages = getCurrentLanguage(browser);
  console.log(`現在の${browserName}言語設定:`, currentLanguages || "なし（システムデフォルト）");

  saveBackup(browser, currentLanguages);
  setLanguage(browser, targetLocale);

  const newLanguages = getCurrentLanguage(browser);
  console.log(`${browserName}の言語設定を${targetLocale}に変更しました:`, newLanguages);
}

async function globalSetup() {
  if (platform() !== "darwin") {
    console.log("Linux/CI環境を検出しました。環境変数LC_ALLで言語設定を制御します。");
    return;
  }
  ...
  console.log(`目標ロケール: ${targetLocale} (LC_ALL=${process.env.LC_ALL || "デフォルト"})`);
  ...
  try {
    ...
    console.warn(`   defaults write org.chromium.Chromium AppleLanguages '("${targetLocale}")'`);
  }
}

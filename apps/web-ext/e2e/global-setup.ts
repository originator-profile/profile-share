...
async function globalSetup() {
  ...
  if (platform() === "darwin") {
    ...
    try {
      ...
      // Chromium用の日本語設定を適用
      execSync("defaults write org.chromium.Chromium AppleLanguages '(\"ja-JP\")'");
      ...
      try {
        ...
        execSync("defaults write com.google.Chrome AppleLanguages '(\"ja-JP\")'");

        const newChromeLanguages = execSync(
          "defaults read com.google.Chrome AppleLanguages 2>/dev/null",
          { encoding: "utf-8" }
        ).trim();
        console.log("✅ Chromeの言語設定も日本語に変更しました:", newChromeLanguages);
      } catch {
      ...
      console.warn("   defaults write org.chromium.Chromium AppleLanguages '(\"ja-JP\")'");
    }
  } else {
    console.log("Linux/CI環境を検出しました。環境変数LC_ALLで制御しているため、言語設定の変更は行いません。");
  }
}

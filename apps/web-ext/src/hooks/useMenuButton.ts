...
export function useMenuButton({ onItemSelect, items }: UseMenuButtonOptions) {
  ...
  const handleMenuKeyDown = useCallback((event: React.KeyboardEvent, itemValue: string) => {
    switch (event.key) {
      case 'Enter':
      case ' ':
        event.preventDefault();
  ...
  }, [selectItem, focusNextItem, focusPrevItem, focusFirstItem, focusLastItem, closeMenu]);
